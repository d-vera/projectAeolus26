...
## Purpose
Manages user UI preferences stored in a dedicated `user_preferences` database table with type-safe `Theme` (`DARK`, `LIGHT`, `SYSTEM`) and `Language` (`ES`, `EN`) Enums, defaulting to `Theme.SYSTEM` and `Language.ES` (Option A).
