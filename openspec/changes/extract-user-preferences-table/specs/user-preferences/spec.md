...
- **THEN** the system SHALL initialize default preferences with `theme = SYSTEM`, `language = ES`, and `active = true`
...
- **WHEN** an authenticated user sends a PATCH request to `/api/preferences/me` with `{"theme": "LIGHT", "language": "ES", "active": true}`
